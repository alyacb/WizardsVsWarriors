...
public class MainActivity extends Activity implements OnClickListener {
    ...
    boolean right, first = true;

    boolean sendingConcentration, sendingMellow, connectedToServer = false;

    String mServerMessage;

    private Socket socket;
    private static final int SERVERPORT = 4454;
    private static final String SERVER_IP = "10.129.252.207";
...
//        double arr[] = new double[100];
        ...
        private void updateConcentration(final ArrayList<Double> data) {
            if (!sendingConcentration) return;
            Activity activity = activityRef.get();
//            Log.e("CONCENTRATION", data.get(0)+"");
            ProgressBar progress = (ProgressBar) findViewById(R.id.progress);
            Double d = 10*data.get(0);
...
//            Log.e("CONCENTRATION", data.get(0)+"");
        ...
        private void updateAlphaRelative(final ArrayList<Double> data) {
            Activity activity = activityRef.get();
            final TextView tt = (TextView) findViewById(R.id.version);
            if (activity != null) {
                activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                         ...
                         elem4.setText(String.format(
                            "%6.2f", data.get(Eeg.TP10.ordinal())));
                        if ((data.get(Eeg.TP9.ordinal())+"").equalsIgnoreCase("NaN") ||
                                (data.get(Eeg.FP1.ordinal())+"").equalsIgnoreCase("NaN") ||
                                (data.get(Eeg.FP2.ordinal())+"").equalsIgnoreCase("NaN") ||
                                (data.get(Eeg.TP10.ordinal())+"").equalsIgnoreCase("NaN")){
                            tt.setBackgroundColor(getResources().getColor(R.color.background_red));
                        } else
                            tt.setBackgroundColor(getResources().getColor(R.color.background_green));
                    }
                });
            }
        }
...
//        scooby.execute();
        ...
        findViewById(R.id.concentration_button).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                setSendingThing(true);
            }
        });

        findViewById(R.id.mellow_button).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                setSendingThing(false);
            }
        });

        mProgressBar = (ProgressBar) findViewById(R.id.mellow_progress);
    }

    private void setSendingThing(boolean concetration){
        if (sendingConcentration) return;
        if (sendingMellow) return;

        TextView textView = (TextView) findViewById(R.id.sending_thing);

        if (concetration){
            textView.setText("Concentration");
            sendingConcentration = true;
        } else {
            textView.setText("Calmness");
            sendingMellow = true;
        }
    }
                        ...
                        if (mServerMessage != null) {
                            ...
                            MainActivity.this.runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    try{
                                        if(right){
                                            mProgressBar.setProgress(Integer.parseInt(mServerMessage));
                                        } else {
                                            mProgressBar.setProgress(2000-Integer.parseInt(mServerMessage));
                                        }
                                    } catch (NumberFormatException e) {
                                        if(mServerMessage!=null){
                                            Log.e("LogTag", mServerMessage);
                                            TextView resultTT = (TextView) findViewById(R.id.result_textview);
                                            if (mServerMessage.equalsIgnoreCase("win") && resultTT.getVisibility() == View.GONE){
                                                resultTT.setText("You Won!");
                                                resultTT.setVisibility(View.VISIBLE);
                                            }
                                            if (mServerMessage.equalsIgnoreCase("lose") && resultTT.getVisibility() == View.GONE){
                                                resultTT.setText("You Lost :(");
                                                resultTT.setVisibility(View.VISIBLE);
                                            }
                                        }
                                    }
